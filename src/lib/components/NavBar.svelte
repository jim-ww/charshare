<script lang="ts">
	...
	const displayName = $derived(
		profile?.username || (user ? `${user.slice(0, 6)}…${user.slice(-4)}` : '')
	);
</script>

<nav class="navbar bg-base-200 px-4">
	<div class="flex flex-1 items-center gap-4">
		<a href="/characters" class="text-lg font-semibold">charshare</a>
		<a href="/characters" class="link link-hover">Characters</a>
		<a href="/chats" class="link link-hover">Chats</a>
	</div>
	<div class="flex items-center gap-2">
		<a href="/characters/new" class="btn btn-sm btn-primary">+ New Character</a>
		<button
		...
		<button class="btn btn-sm btn-ghost" type="button" onclick={() => openSettings('profile')}>
			{displayName}
		</button>
	</div>
</nav>
